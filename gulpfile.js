...
function cleanForPublish(cb) {
    shell.rm('-rf', ['./Reports', './dist/config', './dist/tests', './dist/products', './ts-defs/products', './ts-defs/tests']);
    cb();
}

function copyFeatureFiles() {
    return gulp.src('./src/**/specs/**/*.feature')
        .pipe(gulp.dest('./dist/'));
}
...
function setSemVer(semVer) {
    shell.sed('-i', /"version": "(.*)",/, `"version": "${semVer}",`, ['package.json']);
}
...
function updateSchemas(cb) {
    ...
    while (schemasToGenerate.length !== 0) {
        ...
        const contents = prettier.format(JSON.stringify(schema), { parser: 'json' });
...
function watch(cb) {
    exports.watchBuild((err) => {
        notify(`Build ${err ? 'failed' : 'passed'}`, err ? err.message.toString() : 'Completed with no errors');
        cb();
    });
}
...
exports.clean = clean;
exports.test = test;
exports.copyFiles = parallel(copyJavascriptFiles, copyFeatureFiles, copyResourceFiles, copyConfig);
exports.build = series(clean, lint, buildTypescript, exports.copyFiles);
exports.updateSchemas = updateSchemas;
exports.ci = series(exports.build, test, exports.copyFiles, cleanForPublish, checkVersioning, updateSchemas);
